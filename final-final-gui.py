...
def download(url, file_folder, temp_name):
    ...
    with open(file_path, "ab") as f:
        for chunk in r.iter_content(chunk_size=1024 * 1024 * 8):
            ...
            if chunk:
                ...
                info = "{}% {} KB/S".format(done*2, int(len(chunk) / 1024 / 1024 / (time.time() - startTime)))
                var_down.set(info)
    ...
    global var_3
    if need_down:
        tkinter.messagebox.showinfo('提示', '下载完成，请先手动【断网】，后点击【安装】')
        var_3.set('请先手动【断网】，后点击【安装】')
    else:
        tkinter.messagebox.showinfo('提示', '下载完成，请点击【安装】')
...
def startDown():
    ...
    if valid() == False:
        tkinter.messagebox.showwarning('提示', '注册码错误，请点击【下载】按钮重新输入')
        return
    ...
    global file_name
    file_name = sys_soft_list[chosen_down]['FileName']
    global down_link
    for i in range(5):
...
btn_install = ttk.Button(down_frame, text='安装', state=DISABLED, command=installSoft)
# 状态信息
